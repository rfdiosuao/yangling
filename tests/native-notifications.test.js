...
import { buildNotifications, syncNativeReminder, REMINDER_ID } from '../src/mobile/native-notifications.js'
...
describe('Android offline reminders', () => {
  it('schedules the selected interval and cancels only its own reminder', async () => {
    ...
    expect(plugin.schedule.mock.calls[0][0].notifications[0].schedule).toMatchObject({ on: { hour: 8, minute: 0 }, repeats: true })
    expect(plugin.schedule.mock.calls[0][0].notifications[1].schedule.on).toEqual({ hour: 8, minute: 30 })
    await syncNativeReminder({ enabled: false }, '小养', { plugin })
    expect(plugin.cancel.mock.calls.at(-1)[0].notifications).toContainEqual({ id: REMINDER_ID })
  })
  it('does not schedule after denied permission or postpone an existing matching schedule', async () => {
    ...
    const plugin = device('granted', buildNotifications({enabled:true,intervalMin:45}, '朋友'))
