...
export default function KnowledgeScreen({config,mode,notify,onDialect}){
  ...
  return <main className="ylm-page ylm-knowledge"><h1>{mode==='parent'?'有问题，问一问':'养生说法靠不靠谱？'}</h1><p className="ylm-subtitle">查到依据，再给你简单回答。</p>
    ...
    {sources&&<Sheet title="知识依据" onClose={()=>setSources(false)}>{answer.sources.length?answer.sources.map((s,i)=><article className="ylm-source-detail" key={i}><h3>[{i+1}] {s.title}</h3>{s.kind==='research'&&<small>用户导入的论文摘录 · {s.id} · 非个体建议</small>}<p>{s.body}</p>{/^https?:\/\//.test(s.sourceUrl||'')&&<a href={s.sourceUrl} target="_blank" rel="noreferrer">查看来源原文</a>}</article>):<p>本次没有可引用的知识条目。</p>}</Sheet>}
  </main>
}
